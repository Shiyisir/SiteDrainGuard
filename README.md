...
发布状态和验证证据见 [`AUDIT_REPORT.md`](AUDIT_REPORT.md)。独立复验后的本地验证在 Windows/Python 3.11.12 上通过 19 个测试，覆盖率 85%；GitHub 托管 CI 在 Ubuntu / Python 3.11.16 上同样通过 19 个测试、85% 覆盖率及真实 Heavy 批处理和 Hybrid audit。[运行证据](https://github.com/Shiyisir/SiteDrainGuard/actions/runs/34111256789)。

Release status and verification evidence are recorded in [`AUDIT_REPORT.md`](AUDIT_REPORT.md). Local post-review verification passed 19 tests with 85% coverage on Windows/Python 3.11.12. Hosted GitHub Actions also passed 19 tests with 85% coverage, the real Heavy batch and Hybrid audit on Ubuntu / Python 3.11.16. [Run evidence](https://github.com/Shiyisir/SiteDrainGuard/actions/runs/34111256789).
